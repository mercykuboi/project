...
<!-- for retrieving data -->
				<?php 
							include('online.php');
							session_start();
                           if ($result = mysqli_query($connect, "SELECT * FROM search_result,patient WHERE
						   user='" . $_SESSION["user"] . "' ")) {
		...
		<h3 class="text-center" style="background-color:#272327;color: #fff;">Book Appointment</h3>
			<div class="formstyle" style="float: right;width:500px;padding:20px;margin-right:400px; margin-bottom:30px;background-color:grey;color:#141313;">
				...
				Doctor Name: <input type="text" name="dcontact" value="<?php echo $name; ?>"disabled />
					...
					<label>
						user: <input type="text" name="user" value="<?php echo $user; ?>"required />
					...
					<label>
 					...
 					<label>
						 Contact: <input type="text" name="contact" value="<?php echo $pcontact; ?>"required/>
					</label><br><br>
					<label>
						 E-mail: <input type="email" name="email" value="<?php echo $pemail; ?>" required/>
					</label><br><br>
 					
					<label>
						 Address: <input type="text" name="address" value="<?php echo $paddress; ?>" required>
					</label><br><br>
					<label>
...
<br><br><br>
			...
			<?php
						...
						$sql = " INSERT INTO booking (dname,dcontact,expertise,user, pname,contact,email,
						address,dates,time,payment)
							VALUES ('$name', '$contact', '$expertise','" . $_POST["user"] . "'
							,'". $_POST["pname"] . "',
